...
#80/20 train split ELN alpha scan for the KNN imputated beta-matrix
...
#Loading rhe top 100 feature matrix (X) and outcome (y)

X_train_knn_100_80_20 <- readRDS("X_train_80_20_knn_imputed.rds")
...
all_models_knn_100 <- lapply(alpha_table, fit_one_alpha, feature_matrix = X_train_knn_100_80_20)
...
auc_knn_100 <- sapply(all_models_knn_100, function(fit) {p <- as.numeric(predict(fit, newx = X_train_knn_100_80_20, s = "lambda.min", type = "response")); as.numeric(auc(roc(response = y_train_80_20_factor, predictor = p)))})
