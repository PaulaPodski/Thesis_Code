...
#80/20 train split ELN alpha scan for the mean imputated beta matrix (Top 20)
...
all_models_mean_20 <- lapply(alpha_table, fit_one_alpha, feature_matrix = X_train_mean_20_80_20)
...
auc_mean_20 <- sapply(all_models_mean_20, function(fit) {p <- as.numeric(predict(fit, newx = X_train_mean_20_80_20, s = "lambda.min", type = "response")); as.numeric(auc(roc(response = y_train_80_20_factor, predictor = p)))})

n_retained_mean_20 <- sapply(all_models_mean_20, function(fit) { sum(as.matrix(coef(fit, s = "lambda.min"))[-1, 1] != 0)})

deviance_mean_20 <- sapply(all_models_mean_20, function(fit) min(fit$cvm))

#Creating the final results table:

results_mean_20 <- data.frame(alpha = alpha_table, AUC = auc_mean_20, no_CpGs_retained = n_retained_mean_20, deviance = deviance_mean_20)

print(results_mean_20)

#Plotting

png("alpha_vs_apparent_AUC_mean_top20.png", width = 1800, height = 1400, res = 300)

plot(results_mean_20$alpha, results_mean_20$AUC, type = "b", pch = 19, col = "blue", xlab = "Alpha (0 = Ridge, 1 = Lasso)", ylab = "Apparent train AUC at lambda.min", main = "80/20 Mean Top 20: Alpha vs AUC", ylim = c(0.5, 1.02))

abline(h = 0.5, lty = 2, col = "grey")

dev.off()

png("alpha_scan_80_20_mean_top20_AUC.png", width = 1800, height = 1400, res = 300)
plot(results_mean_20$alpha, results_mean_20$AUC, type = "b", pch = 19, col = "blue",
     xlab = "alpha value", ylab = "AUC (lambda.min)",
     main = "Elastic Net Alpha Scan, 80/20 Train Top 20 (Mean Imputed)")
dev.off()

png("alpha_scan_80_20_mean_top20_AUC.png", width = 1800, height = 1400, res = 300)
plot(results_mean_20$alpha, results_mean_20$AUC, type = "b", pch = 19, col = "black",
     xlab = "alpha value", ylab = "AUC (lambda.min)",
     main = "Elastic Net Alpha Scan, 80/20 Train Top 20 (Mean Imputed)",
     cex.main = 0.8)
dev.off()
