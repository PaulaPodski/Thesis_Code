...
test_file.list <- readRDS("test_file_list_80_20_min12.rds")
test_ids       <- split$test_ids
test_treatment <- split$test_treatment
stopifnot(length(test_file.list) == length(test_ids),
          length(test_ids) == 14,
          sum(test_treatment) == 7, sum(test_treatment == 0) == 7)
myobj_test <- methRead(
  test_file.list,
  ...
  mincov = 10,
  dbtype = "tabix",
  dbdir = "methylDB_test_80_20_min12"
)

filtered_test   <- filterByCoverage(myobj_test, lo.count = 10, lo.perc = NULL,
                                     hi.count = NULL, hi.perc = 99.9)
normalized_test <- normalizeCoverage(filtered_test)
saveRDS(normalized_test, "normalized_test_80_20_min12.rds")

meth_test <- unite(normalized_test, destrand = FALSE, min.per.group = 4L) #min.per.group 4L used here 
