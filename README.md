# Thesis Title: Machine Learning Classifiers to Predict Prostate Cancer Mortality from Whole Genome Bisulfite Sequencing Data 
...
#Key methodological point: train/test partitions are processed independently through the full methylKit workflow (feature selection, coverage filtering, imputation) to avoid data leakage 
# Pipeline stages
# Stratified train/test split (70/30 and 80/20), set.seed(42)
# Independent methylKit processing per split (methRead → filterByCoverage → normalizeCoverage → unite)
